...
\footnotetext{une popup est une fenêtre (modale) qui apparaît au-dessus de la page et elle est, en général, utilisée comme une boîte de dialogue avec l’utilisateur.}.
...
- Nous interrogeons le web service AppTour avec une requête Ajax en passant en paramètre le nom de l'application et l'identifiant de la page courante. La requête vérifie s'il existe de la documentation pour la page concernée. 
...
\item \textbf{V3: Sélection des composants et l'ajout de la documentation}\\ 

\end{enumerate}

Maintenant que l'ADSI peut déclencher la documentation à l'aide d'un bouton, nous avons travaillé sur la V3. Il s'agit d'ajouter un \textbf{"mode édition"} dans l'application Solycee. 
...
\item Ajouter ou modifier la documentation : Quand l'ADSI est en \textbf{"mode édition"}, il peut soit ajouter la documentation pour un composant de la page web dans le cas où il n'existe pas de documentation pour ce dernier, soit la modifier dans le cas où elle existe. 
...
\footnotetext{Spring RestTemplate est une classe de Spring qui permet d'établir une communication entre un client et un serveur REST, ceci grâce aux requêtes HTTP.}.\\
