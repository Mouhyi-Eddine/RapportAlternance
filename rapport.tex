...
- Ce mode consiste à rester la même page web où il se trouvait l'ADSI mais le comportement des composants(boutons, url,logo,etc...) de cette page change et ne fonctionne plus comme le mode normal. 

Si l'ADSI appuie sur un composant de la page alors qu'il est en mode édition, cela affiche une modale qui contient plusieurs composants(Titre,Élément,Contenu...) créés en HTML et CSS.Voir Figure 9.

\begin{itemize}
\item \textbf{Élement: } Id de l'élément séléctionné. 
\item \textbf{Title: } Titre affiché sur la popup. 
\item \textbf{Content: } Message affiché dans pour expliqué le fonctionnement du composant appuyé.  
\end{itemize}
...
- Si l'étape existe dans la table Step de AppTour,nous récupèrons l'objet et puis nous remplissons les champs par les données récuéprées(Title \& content).

- Dans le cas où il n'exsite pas de documentation pour l'élément séléctionné, nous laissons les champs vides. 

\item Rajouter ou modifier la documentation : Quand l'ADSI est en mode édition, il peut soit rajouter la documentation pour un composant de la page web dans le cas où il n'existe pas de docuementation pour ce dernier, soit la modifier dans le cas où elle existe. 
